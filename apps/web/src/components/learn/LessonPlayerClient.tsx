...
export default function LessonPlayerClient() {
  ...
  const backHref = courseSlug ? `/learn/course/${courseSlug}` : `/learn/hsk/${level}`;
  const backLabel = courseSlug === "pinyin" ? "Phát âm nền" : courseSlug ? courseSlug : `HSK ${level}`;
  const nextLessonHref = (nid: number) =>
    courseSlug ? `/learn/course/${courseSlug}/${nid}` : `/learn/hsk/${level}/${nid}`;
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {step?.step_key === "complete" && next_lesson_id && (
                <button
                  ...
                  onClick={() => router.push(nextLessonHref(next_lesson_id))}
                >
                  Bài tiếp theo
                </button>
              )}
              {step?.step_key === "complete" && courseSlug === "pinyin" && !next_lesson_id && (
                <Link href="/learn/hsk/1" className="btn btn-navy flex-1 text-center text-sm">
                  Vào HSK 1 →
                </Link>
              )}
            </div>
...
function StepBody({
  ...
  if (step.step_key === "objectives" || step.step_key === "review" || step.step_key === "grammar" || step.step_key === "phonics") {
    return (
        ...
        <ul className="mt-4 space-y-4">
          {items.map((it, i) => (
            <li key={it.id} className="flex gap-3">
              ...
              <div>
                {it.speaker && (
                  <p className="text-sm font-bold text-[var(--navy)]">{it.speaker}</p>
                )}
  ...
  if (step.step_key === "practice" && item?.item_type === "quiz_prompt") {
    ...
    return (
      <div className="rounded-md border border-[var(--line)] bg-white p-5 shadow-md md:p-8">
        ...
        <p className="mt-4 text-center text-base font-semibold text-[var(--ink)]">
          {item.meaning_vi || "Chọn đáp án đúng"}
        </p>
